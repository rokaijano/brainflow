...
constexpr int MindRove::num_channels;


MindRove::MindRove (struct BrainFlowInputParams params)
...
int MindRove::config_board (char *config)
{
    safe_logger (spdlog::level::err, "Notion OSC doesnt support config_board");
    return (int)BrainFlowExitCodes::UNSUPPORTED_BOARD_ERROR;
}

int MindRove::start_stream (int buffer_size, char *streamer_params)
{
    if (!initialized)
    {
...
void MindRove::handle_packet (const OSCPP::Server::Packet &packet)
{
    if (packet.isBundle ())
    {
